      ...
      <div className="section">
        <div className="mTitle" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div className="gRight" style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <a id="eBtnSearch" onClick={() => router.push('/admin/bbs/post/add')} className="btnSearch" style={{ marginRight: '10px' }}><span>작성</span></a>
              ...
              {list && list.length > 0 ? (
                  list.map((ar, i) => (
                    <tr key={i} style={{textAlign: 'center'}}>
                        ...
                        <td onClick={() => router.push(`/admin/bbs/post/detail/${ar.boardkey}`)} style={{ cursor: 'pointer' }}>
                          {(page.nowPage - 1) * page.numPerPage + i + 1}
                        </td>
                        <td>{categoryMap[ar.categorykey] || 'X'}</td>
                        <td onClick={() => router.push(`/admin/bbs/post/detail/${ar.boardkey}`)} style={{ cursor: 'pointer' }}>{ar.title}</td>
                        <td>{ar.create_dtm}</td>
                        <td>{ar.update_dtm}</td>
                        <td>{ar.viewqty}</td>
                    </tr>
